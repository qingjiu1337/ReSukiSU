...
use crate::{
    android::{
        ...
        profile, sepolicy, su, sulog, susfs, uapi, umount_config, utils, webui,
    },
...
enum Commands {
    ...
    /// Run WebUI panel daemon (127.0.0.1 only). Started automatically by `ksud services`.
    #[command(hide = true)]
    Webuid,
...
pub fn run() -> Result<()> {
    ...
    let result = match cli.command {
        ...
        Commands::Services => {
            ...
            // 拉起 WebUI 面板守护进程（webuid，仅监听 127.0.0.1）
            if let Err(e) = webui::ensure_webuid_running() {
                error!("failed to ensure webuid is running: {e:#}");
            }
            Ok(())
        }
        Commands::Sulogd => sulog::run_sulogd(),
        Commands::Webuid => webui::run_webuid(),
