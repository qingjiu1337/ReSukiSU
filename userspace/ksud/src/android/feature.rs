...
impl FeatureId {
    ...
    pub const fn description(self) -> &'static str {
        match self {
            ...
            Self::Stealth => {
                "Stealth Mode - tighten dmesg_restrict/kptr_restrict and silence KernelSU kernel logs"
            }
        }
    }
}
...
fn parse_feature_id(name: &str) -> Result<FeatureId> {
    match name {
        ...
        "stealth" | "5" => Ok(FeatureId::Stealth),
        _ => bail!("Unknown feature: {name}"),
    }
}

/// 供其他模块（如 WebUI 面板）复用的 feature 设置入口，
/// 包含 Sulog 拉起与 Stealth 加固等副作用处理。
pub fn apply_feature(feature_id: FeatureId, value: u64) -> Result<()> {
    set_kernel_feature(feature_id, value)
}

/// Stealth 模式的用户态兜底：
/// 内核侧通过 kallsyms 解析 dmesg_restrict / kptr_restrict 并直接改写，
/// 若符号解析失败（如 CONFIG_KALLSYMS_ALL=n），则由 ksud 写 /proc/sys 完成同样的加固。
fn apply_stealth_hardening(enable: bool) {
    use std::io::Write as _;

    let entries: &[(&str, &str)] = if enable {
        &[
    ...
    for (path, value) in entries {
        ...
        {
            Ok(()) => log::info!("stealth hardening: {path} -> {value}"),
            Err(e) => log::warn!("stealth hardening: failed to write {path}: {e}"),
        }
    }
}
...
fn set_kernel_feature(feature_id: FeatureId, value: u64) -> Result<()> {
    ...
    if feature_id == FeatureId::Stealth {
        apply_stealth_hardening(value != 0);
    }
